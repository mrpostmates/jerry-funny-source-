...
class PlayState extends MusicBeatState
{
	...
	public static var lua:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'lampshade':
				dialogue = CoolUtil.coolTextFile(Paths.txt('lampshade/jerrygaming'));
			case 'sock':
				dialogue = CoolUtil.coolTextFile(Paths.txt('sock/jerrygaming'));
			case 'fan':
				dialogue = CoolUtil.coolTextFile(Paths.txt('lampshade/jerrygaming'));
			case 'alcohol':
				dialogue = CoolUtil.coolTextFile(Paths.txt('lampshade/jerrygaming'));
		...
		else if (SONG.song.toLowerCase() == 'senpai' || SONG.song.toLowerCase() == 'roses')
		{
		...
		else if (SONG.song.toLowerCase() == 'lampshade'){
			
			curStage = 'jerryStage';
			
			var bg:FlxSprite = new FlxSprite( -296.05, -270.1);
			bg.loadGraphic(Paths.image("jerry/theback™"));
			add(bg);
			bg.active = false;
			bg.updateHitbox();
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);		
				...
				else if (SONG.song.toLowerCase() == 'sock'){
		...
		else if (SONG.song.toLowerCase() == 'alcohol'){
			
			curStage = 'jerryStage';
			
			var bg:FlxSprite = new FlxSprite( -296.05, -270.1);
			bg.loadGraphic(Paths.image("jerry/drunk_bg"));
			add(bg);
			bg.active = false;
			bg.updateHitbox();
			bg.antialiasing = true;
		...
		else if (SONG.song.toLowerCase() == 'ball-itcher'){
			
			curStage = 'jerryStage';
			
			var bg:FlxSprite = new FlxSprite( -296.05, -270.1);
			bg.loadGraphic(Paths.image("jerry/chaos"));
			add(bg);
			bg.active = false;
			bg.updateHitbox();
			bg.antialiasing = true;
		...
		else if (SONG.song.toLowerCase() == 'fan'){
			
			curStage = 'jerryStage';
			
			var bg:FlxSprite = new FlxSprite( -296.05, -270.1);
			bg.loadGraphic(Paths.image("jerry/🎵"));
			add(bg);
			bg.active = false;
			bg.updateHitbox();
			bg.antialiasing = true;
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'momiStage':
				boyfriend.setPosition(825.1, 294.65);
				gf.setPosition(372.8, -27.3);
		}
